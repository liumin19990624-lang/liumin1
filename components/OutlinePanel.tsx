  ...
  const handleStart = async () => {
    ...
    try {
      ...
      if (analysisMode === 'CHARACTERS') {
        stream = gemini.extractCharactersStream(file?.content || '');
      } else if (analysisMode === 'REF_SCRIPT') {
        stream = gemini.extractReferenceScriptStream(file?.content || '');
      } else if (analysisMode === 'CAST_BIO') {
        stream = gemini.generateCharacterBioStream(file?.content || '');
      } else {
        stream = gemini.generateFullOutlineStream(AudienceMode.MALE, file?.content || '', isDeepExtraction);
      }
           ...
           <div className="max-w-2xl w-full bg-white/[0.03] border border-white/10 rounded-[3rem] p-12 backdrop-blur-3xl shadow-2xl">
              ...
              <div className="space-y-8">
                <div className="space-y-3">
                  ...
                  <select value={targetId} onChange={e => setTargetId(e.target.value)} className="w-full p-5 bg-black/40 border border-white/10 text-white rounded-2xl outline-none font-bold text-sm focus:border-blue-500 transition-all">
                ...
                <button disabled={isLoading || !targetId} onClick={handleStart} className="w-full bg-blue-600 hover:bg-blue-500 disabled:bg-slate-800 text-white p-6 rounded-3xl font-black text-sm uppercase transition-all shadow-2xl flex items-center justify-center gap-3">
